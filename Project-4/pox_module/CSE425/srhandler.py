...
class SRServerListener(EventMixin):
  ...
  def __init__ (self, address=('127.0.0.1', 8888)):
    port = address[1]
    self.listenTo(core.CSE425_ofhandler)
    self.srclients = []
  ...
  def _handle_packet_msg(self, conn, vns_msg):
    ...
    core.CSE425_srhandler.raiseEvent(SRPacketOut(pkt, out_port))
...
class CSE425_srhandler(EventMixin):
...
def launch (transparent=False):
  ...
  """
  core.registerNew(CSE425_srhandler)
